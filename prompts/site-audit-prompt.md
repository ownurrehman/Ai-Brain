> **Parent Hub:** [[prompts/INDEX|🎯 Prompts Hub]] · [[INDEX|🧠 Ai Brain]]

# Task: Comprehensive Site Audit

**Role**: Senior SEO Strategist, UX Auditor, and Accessibility Expert.
**Task**: Perform a comprehensive audit for `[Site URL]`.

---

## 1. Boot (DO THIS FIRST)

1. Read `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/INDEX.md` — find the project.
2. Read `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/projects/{name}/mastersheet.md` — tone, entities, status.
3. Read `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/master-env.env` — get credentials.
4. Read `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/rules/content/content-rules.md` — quality standards.

## 2. Skills to Load

- `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/rules/content/content-rules.md`
- Master content skill: `~/.hermes/profiles/enigma/skills/seo/rankray-seo-content-mastery/SKILL.md` (covers audits, technical SEO, cannibalization — reference file `audits-technical-refresh.md`; former `skills/seo-audit`, `seo-technical`, `seo-cannibalization-detector` consolidated 2026-08-28)
- `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/skills/accessibility-compliance-accessibility-audit/SKILL.md` *(check exists; may live in antigravity submodule)*
- `/Users/sheikhown/Ai Works - Local/Ai Codes/Ai Brain/skills/security-audit/SKILL.md` *(check exists; may live in antigravity submodule)*

## 3. Execution

1. **Crawler Emulation**: Analyze site structure, URL patterns, internal linking.
2. **Technical Scan**: Status codes, redirects, schema markup, meta tags.
3. **UX Walkthrough**: Friction points, broken layouts, slow-loading elements.
4. **Content Quality Check**: E-E-A-T signals, thin content, cannibalization.
5. **Report**: Critical Issues / Opportunities / Strengths + Action Plan.
