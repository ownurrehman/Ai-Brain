...
async function appendToSheet(token, values) {
  // Use append API instead of update - more robust
  const { data } = await request({
...
async function main() {
  console.log(`=== GMB Lead Finder [${COUNTRY}] - ${new Date().toISOString()} ===`);
  ...
  // Pick random 10 city+industry combos for this country
  ...
  for (const { city, industry } of dailyBatch) {
    ...
    const lead = [
      ...
      `No website found for ${industry} in ${city}, ${COUNTRY}. Missing online presence.`,
      '', 'B', 'New Lead', '', 'No', 'Pending', '', '',
      `Found via daily GMB search. Requires manual verification. Search: "${industry} ${city} phone"`,
      '', '', '', '', '', '', '',
    ];
    
    newLeads.push(lead);
    console.log(`  Added: ${leadId} | ${industry} | ${city}`);
  ...
  if (newLeads.length > 0) {
    ...
    console.log(`\n✓ Appended ${newLeads.length} ${COUNTRY} leads starting at row ${startRow}`);
  }
