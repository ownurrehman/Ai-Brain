...
if [ -f "$ENV_FILE" ]; then
    export $(grep -E '^TONICPHYSIO_WP' "$ENV_FILE" | xargs)
fi

WP_USER="${TONICPHYSIO_WP_USER:?ERROR: TONICPHYSIO_WP_USER not set in master-env.env}"
WP_PASS="${TONICPHYSIO_WP_APP_PASS:?ERROR: TONICPHYSIO_WP_APP_PASS not set in master-env.env}"
