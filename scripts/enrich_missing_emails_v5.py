...
async def curl_fetch_async(url):
    """Non-blocking curl with asyncio subprocess."""
    try:
...
async def fetch_all_pages(start_url):
    ...
    homepage = await curl_fetch_async(start_url)
    if not homepage:
        return ""
    all_html = homepage
    try:
        ...
        for path in ["/contact", "/contact-us"]:
            page = await curl_fetch_async(urljoin(base, path))
            if page:
                all_html += " " + page
    except Exception:
        pass
...
async def process_one(item):
    ...
    html = await fetch_all_pages(item["website"])
    emails = find_emails(html)
    best = pick_best_email(emails, domain)
    status = "✅" if best else ("🌐" if html else "❌")
    log(f"[{item['idx']}/{item['total']}] Row {item['row']}: {item['business'][:45]} | {status} {best or emails[:3] or 'no email'}")
    return {
        "row": item["row"],
...
async def main():
    ...
    # Process concurrently with semaphore to limit load
