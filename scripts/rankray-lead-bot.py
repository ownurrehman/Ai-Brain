...
def generate_sample_leads(count=20, existing_names=None):
    ...
    for _ in range(count):
        ...
        lead_id = f"RR-{date_str.replace('-', '')}-{suffix}"
...
def main():
    ...
    print(f"Found {len(existing_names)} existing leads")
    ...
    while total_added < target:
        ...
        total_added += added
        print(f"Added {added} leads (total: {total_added})")
        
        if added < batch_size:
            print("Running out of unique names, stopping...")
            break
    
    print(f"\nDone! Added {total_added} new leads.")
    print(f"Total leads in sheet: {len(existing_names) + total_added}")
