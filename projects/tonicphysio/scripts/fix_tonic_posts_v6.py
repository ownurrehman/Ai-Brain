...
v6 - Precise FAQ detection, proper sibling handling, preserves Common Questions Answered.
"""
...
def is_faq_heading(text):
    """Check if text is an FAQ heading (not Common Questions Answered)."""
    text_lower = text.lower().strip()
    # Match FAQ-related headings
    faq_patterns = [
...
def find_faq_heading(soup):
    """Find the FAQ heading h2/h3 element."""
...
def extract_faq_items(faq_h):
    """Extract FAQ Q\u0026A pairs from siblings after FAQ heading."""
    faqs = []
    for sibling in faq_h.find_next_siblings():
        ...
        if sibling.name == 'p':
            ...
            if strong_tag:
                ...
                # Get full text and remove question part
                full_text = sibling.get_text(strip=True)
                question_text = strong_tag.get_text(strip=True)
                answer = full_text.replace(question_text, '').strip()
...
def remove_faq_section(content):
    """Remove FAQ heading and its content."""
    ...
    faqs = extract_faq_items(faq_h)
    
    # Remove heading and all siblings until next h2/h3
    to_remove = [faq_h]
    for sibling in list(faq_h.find_next_siblings()):
        if sibling.name in ['h2', 'h3']:
            break
    ...
    return new_content, True, f"FAQ removed ({removed} chars)", faqs


def ensure_single_blockquote(content, post_title):
    """Remove all summary blockquotes and add one fresh blockquote."""
    soup = BeautifulSoup(content, 'html.parser')
...
def convert_faq_to_h3_sections(content, faqs):
    """Convert FAQ items into H3 subsections under H2 'Common Questions Answered'."""
    if not faqs:
        return content
    ...
    # Build the FAQ section elements
    section_elements = []
    h2 = soup.new_tag('h2')
    h2.string = "Common Questions Answered"
    section_elements.append(h2)
    
    for faq in faqs:
        q_clean = re.sub(r'^[\d\.\s]+', '', faq['question'])
        if not q_clean.endswith('?'):
            q_clean += '?'
        h3 = soup.new_tag('h3')
        h3.string = q_clean
        section_elements.append(h3)
        p = soup.new_tag('p')
        p.string = faq['answer']
        section_elements.append(p)
    
    # Insert before conclusion or append to end
    if insert_before:
...
def main():
    print("="*70)
    print("TONICPHYSIO BLOG POST AEO RESTRUCTURING v6")
    print("="*70)
    ...
    for r in RESULTS:
        if r.get('status') == 'success':
            print(f"  ID {r['id']}: {r['title'][:50]}... | FAQ removed: {r['faq_removed']} | FAQs converted: {r['faq_items_converted']} | Words: {r['original_word_count']} -> {r['new_word_count']}")
        else:
