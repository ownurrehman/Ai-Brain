...
v8 - Fix TOC contamination: extract TOC, process body, reinsert TOC at top.
"""
...
def remove_faq_paragraphs(soup):
    """Remove paragraphs with bold questions and orphaned answers."""
    removed = 0
    to_remove = []
    ...
    # Remove orphaned answer paragraphs
    for faq_p in to_remove:
...
def clean_common_questions_section(soup):
    """Clean up Common Questions Answered section."""
    cqa_h2 = None
    ...
    for sibling in cqa_h2.find_next_siblings():
        if sibling.name == 'h3' and is_question(sibling.get_text(strip=True)):
            ...
            break
        elif sibling.name == 'p' and not sibling.find('strong'):
            last_faq_element = sibling
    
    # Remove elements between last_faq_element and conclusion
...
def ensure_single_blockquote(soup, post_title):
    """Add a single direct-answer blockquote after the first body paragraph."""
    # Remove any existing blockquotes in body (not TOC since TOC is extracted)
    ...
    # Insert after first paragraph in BODY (not in TOC)
    # Find the first paragraph that is NOT inside a nav or TOC-related element
    first_p = None
    for p in soup.find_all('p'):
        # Skip paragraphs inside TOC
        if p.find_parent(id='ez-toc-container'):
            continue
        # Skip paragraphs that are TOC items
    ...
    if first_p:
        ...
        if first_h2:
            first_h2.insert_before(new_bq)
        else:
            soup.append(new_bq)


def ensure_definition_sentence(soup, topic):
...
def ensure_comparison_table(soup, post_title):
...
def process_post(post_id):
    ...
    # Step 1: Extract TOC div and store it
    toc_html = extract_toc(soup)
    
    # Step 2: Remove FAQ paragraphs with bold questions
    removed = remove_faq_paragraphs(soup)
    print(f"Removed {removed} FAQ paragraphs")
    
    # Step 3: Clean Common Questions Answered section
    clean_common_questions_section(soup)
    print("Cleaned Common Questions Answered section")
    
    # Step 4: Ensure single blockquote (inserted into body, not TOC)
    ensure_single_blockquote(soup, title)
    print("Ensured single direct-answer blockquote in body")
    
    # Step 5: Ensure definition sentence
    ensure_definition_sentence(soup, title)
    print("Ensured definition sentence in first H2")
    
    # Step 6: Ensure comparison table
    ensure_comparison_table(soup, title)
    if ' vs ' in title.lower() or 'difference' in title.lower():
        print("Added/verified comparison table")
    
    # Step 7: Re-insert TOC at the beginning
    if toc_html:
    ...
    if update_resp.status_code in (200, 201):
        ...
        RESULTS.append({
            ...
            "title": title,
            "status": "success",
            "faq_removed": removed > 0,
            "faq_paragraphs_removed": removed,
...
def main():
    print("="*70)
    print("TONICPHYSIO BLOG POST AEO RESTRUCTURING v8")
    print("="*70)
