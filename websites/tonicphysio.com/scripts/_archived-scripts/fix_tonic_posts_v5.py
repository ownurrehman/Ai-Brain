...
v5 - Proper ordering, TOC cleanup, verification.
"""
...
def remove_faq_section(content):
    ...
    # Remove the FAQ heading and all elements until next h2/h3
    current = faq_h
    while current:
        next_elem = current.next_sibling
        if hasattr(current, 'name') and current.name in ['h2', 'h3'] and current != faq_h:
            break
...
def ensure_single_blockquote(content, post_title):
    ...
    summary_keywords = ['Key Takeaway', 'Quick Comparison', 'Key Benefits', "What You'll Learn"]
...
def convert_faq_to_h3_sections(content, faqs):
    ...
    # Insert H2 before conclusion
    insert_before.insert_before(new_h2)
    
    # Insert each FAQ pair after the H2 (so they appear between H2 and conclusion)
    last = new_h2
    for faq in faqs:
        q_clean = re.sub(r'^[\d\.\s]+', '', faq['question'])
        if not q_clean.endswith('?'):
...
def main():
    print("="*70)
    print("TONICPHYSIO BLOG POST AEO RESTRUCTURING v5")
    print("="*70)
