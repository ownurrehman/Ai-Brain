...
def remove_faq_section(content):
    """
    Remove FAQ section: find 'Frequently Asked Questions' or 'FAQ' heading
    and everything after it up to the final CTA section.
    """
    original = content
    # Patterns to find FAQ start
    patterns = [
        r'<h[2-6][^>]*>\s*Frequently Asked Questions\s*</h[2-6]>',
        r'<h[2-6][^>]*>\s*FAQ\s*</h[2-6]>',
        r'<h[2-6][^>]*>\s*FAQs\s*</h[2-6]>',
        r'<h[2-6][^>]*>\s*Common Questions\s*</h[2-6]>',
    ]
    ...
    # Find where FAQ section ends - usually before a CTA or Conclusion heading
    # Look for the next major section after FAQ
    remaining = content[faq_start:]

    # Find the end of FAQ - look for next H2 that looks like a conclusion or CTA
    end_patterns = [
        r'<h[2-6][^>]*>\s*(?:Conclusion|Summary|Key Takeaway|Ready to|Book Now|Contact Us|Get Started|Final Thoughts)\s*</h[2-6]>',
    ]

    faq_end = len(content)  # Default: remove to end
    for ep in end_patterns:
        em = re.search(ep, remaining[1:], re.IGNORECASE)  # [1:] to skip the FAQ heading itself
        if em:
            faq_end = faq_start + 1 + em.start()
            break

    # If no end pattern found, look for a CTA div or the last paragraph before </body>
    if faq_end == len(content):
        # Try to find a CTA section or just remove to a reasonable point
        cta_match = re.search(r'(<p[^>]*>.*?book.*?appointment.*?</p>|<a[^>]*href="[^"]*contact[^"]*"[^>]*>.*?</a>)', remaining[50:], re.IGNORECASE)
        if cta_match:
            faq_end = faq_start + 50 + cta_match.start()

    new_content = content[:faq_start].rstrip()
    if faq_end < len(content):
        new_content += "\n\n" + content[faq_end:].lstrip()
...
def extract_faq_items(content):
    """Extract FAQ questions and answers from the content before removal."""
    patterns = [
        r'<h[2-6][^>]*>\s*Frequently Asked Questions\s*</h[2-6]>',
        r'<h[2-6][^>]*>\s*FAQ\s*</h[2-6]>',
        r'<h[2-6][^>]*>\s*FAQs\s*</h[2-6]>',
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            # Find Q&A pairs - look for <h3> or <strong> or <p><strong> patterns
            ...
            h3_matches = list(re.finditer(r'<h3[^>]*>(.*?)</h3>', faq_section, re.DOTALL | re.IGNORECASE))
            for i, h3 in enumerate(h3_matches):
                question = clean_html_entities(re.sub(r'<[^>]+>', '', h3.group(1)))
                start = h3.end()
...
def add_direct_answer_summary(content, post_title, faqs):
    ...
    # Determine summary type based on title
    ...
    # Create summary from post context
    if faqs and len(faqs) > 0:
        # Use first FAQ to create a summary
        summary = f"{faqs[0]['question']} {faqs[0]['answer'][:200]}"
    else:
        # Generic summary based on title keywords
...
"""
    ...
    # Insert after first closing </p> that comes after some content
...
def ensure_definition_sentence(content, topic):
    ...
    h2_what_is = re.search(r'<h2[^>]*>\s*What Is[^<]*</h2>', content, re.IGNORECASE)
    if h2_what_is:
        start = h2_what_is.end()
        ...
        if next_p:
            ...
            if not any(clean_text.lower().startswith(w) for w in ['is a', 'refers to', 'is the', 'is an', 'are a', 'is defined']):
                # Need to add a definition sentence
                definitions = {
...
def convert_faq_to_h3_sections(content, faqs):
    ...
    for faq in faqs:
        q = faq['question']
        a = faq['answer']
        # Clean up the question to make it a proper heading
        q_clean = re.sub(r'^[\d\.\s]+', '', q)  # Remove leading numbers
    ...
    # Insert before the conclusion/CTA
    conclusion_match = re.search(r'<h2[^>]*>\s*(?:Conclusion|Ready to|Book Now|Contact|Get Started|Final Thoughts)', content, re.IGNORECASE)
    if conclusion_match:
...
def ensure_scannable_lists(content):
    """Ensure benefits/steps use proper ul/ol lists."""
    # Find paragraphs that look like lists (start with - or * or numbers)
    # Convert them to proper HTML lists if they aren't already
    # This is a safety check - most content should already have lists
    return content
...
"""
    ...
    # Insert after the first H2 or after the summary blockquote
...
def process_post(post_id):
    """Fetch, process, and update a single post."""
    print(f"\n{'='*60}")
    ...
    # 2. Extract FAQs before removing
...
def main():
    print("="*70)
    print("TONICPHYSIO BLOG POST AEO RESTRUCTURING")
    print("="*70)
