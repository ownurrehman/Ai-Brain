 ...
 */
function justccell_mega_cards_for_category(string $key, array $product_ids = [], int $limit = 5, ?WP_Post $menu_item = null): array
{
    $limit = max(1, min(8, $limit));
    $cards = [];
    $seen  = [];

    $matches = static function (int $pid) use ($menu_item, $key): bool {
        if (function_exists('justccell_product_matches_mega_menu_filters')) {
            return justccell_product_matches_mega_menu_filters($pid, $menu_item, $key);
        }
    ...
    foreach ($product_ids as $pid) {
        $pid = (int) $pid;
        if ($pid < 1 || isset($seen[$pid]) || !$matches($pid)) {
            continue;
        }
    ...
    if (count($cards) < $limit && function_exists('justccell_mega_menu_auto_product_ids')) {
        $auto = justccell_mega_menu_auto_product_ids(
            $menu_item,
            $key,
            $limit - count($cards),
            array_keys($seen)
