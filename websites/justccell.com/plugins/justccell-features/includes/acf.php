...
require_once JUSTCCELL_FEATURES_DIR . 'includes/acf-product-clone-maintenance.php';
...
add_action('admin_init', static function (): void {
    if (get_option('justccell_header_menu_item_acf_v3') === '1') {
        return;
    }
    if (!function_exists('justccell_acf_is_safe_maintenance_request') || !justccell_acf_is_safe_maintenance_request()) {
        return;
    }
    justccell_repair_header_menu_item_acf_group();
}, 26);

/**
 * Rebuild Header item ACF group from Local JSON and remove duplicate field registry rows.
 */
function justccell_repair_header_menu_item_acf_group(): void
{
    if (!function_exists('justccell_acf_repair_field_group_from_local_json')) {
        return;
    }

    justccell_acf_repair_field_group_from_local_json('group_jc_header_menu_item');
    update_option('justccell_header_menu_item_acf_v3', '1', false);
}

/** @deprecated Use justccell_repair_header_menu_item_acf_group() */
function justccell_sync_header_menu_item_acf_group(): void
{
    justccell_repair_header_menu_item_acf_group();
}
