...
function justccell_product_page_from_woo(string $slug): ?array
{
    ...
    if (function_exists('justccell_product_storefront_category')) {
        $resolved = justccell_product_storefront_category($id, $slug);
        if ($resolved !== '') {
            $category = $resolved;
        }
    } else {
...
function justccell_catalog_from_woo(): array
{
    ...
    $append_catalog_row = static function (int $pid, string $cat) use (&$out, &$seen): void {
        ...
        if (function_exists('get_field')) {
            foreach ((array) get_field('clone_specs', $pid) as $row) {
                if (is_array($row) && ($row['line'] ?? '') !== '') {
                    $specs[] = (string) $row['line'];
                }
            }
        ...
        $out[] = [
            ...
            'specs_all'     => $specs,
            'woo_id'        => $pid,
            'mega_featured' => function_exists('get_field') && (bool) get_field('clone_mega_featured', $pid),
    ...
    foreach (justccell_product_category_labels() as $cat => $_label) {
        ...
        foreach ($ids as $pid) {
            $append_catalog_row((int) $pid, $cat);
        }
    }

    $j3_slugs = function_exists('justccell_j3_product_cat_slugs') ? justccell_j3_product_cat_slugs() : [];
    if ($j3_slugs !== [] && function_exists('justccell_product_storefront_category')) {
        $j3_ids = wc_get_products([
            'status'    => 'publish',
            'limit'     => -1,
            'return'    => 'ids',
            'orderby'   => 'menu_order',
        ...
        foreach ($j3_ids as $pid) {
            ...
            $slug = $product->get_slug() !== '' ? $product->get_slug() : $product->get_sku();
            $cat  = justccell_product_storefront_category($pid, (string) $slug);
            if ($cat === '') {
                continue;
            }
            $append_catalog_row($pid, $cat);
        }
    }
