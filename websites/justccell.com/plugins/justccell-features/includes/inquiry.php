...
function justccell_store_lead(string $type, string $title, array $fields): int
{
    ...
    $meta = [
        ...
        '_jc_lead_read'   => '0',
        '_jc_lead_status' => 'new',
        '_jc_lead_email'   => (string) ($fields['Email'] ?? ''),
        '_jc_lead_phone'   => (string) ($fields['Phone'] ?? ''),
        '_jc_lead_company' => (string) ($fields['Company'] ?? ''),
        '_jc_lead_country' => (string) ($fields['Country'] ?? ''),
        '_jc_lead_sku'     => (string) ($fields['SKU'] ?? ''),
        '_jc_lead_name'    => (string) ($fields['Name'] ?? ''),
    ];
 ...
 */
function justccell_send_lead_mail(array $recipients, string $subject, string $body, string $reply_to): void
{
    if ($recipients === []) {
        return;
    }
...
function justccell_handle_inquiry(): void
{
    ...
    $country = function_exists('justccell_form_country_from_submission')
        ? justccell_form_country_from_submission(sanitize_text_field(wp_unslash((string) ($_POST['country'] ?? ''))))
    ...
    $lead_id = justccell_store_lead('inquiry', sprintf('[quote] [%s] %s', strtoupper($account), $company !== '' ? $company : $name), $fields);

    $recipients = function_exists('justccell_form_recipients')
        ? justccell_form_recipients('inquiry')
    ...
    if ($lead_id > 0) {
        $body .= "\n\n" . sprintf(
            /* translators: %s: wp-admin edit URL */
            __('View in WordPress: %s', 'justccell'),
            admin_url('post.php?post=' . $lead_id . '&action=edit')
        );
    }

    justccell_send_lead_mail($recipients, $subject, $body, $email);
...
function justccell_handle_subscribe(): void
{
    ...
    $recipients = function_exists('justccell_form_recipients')
        ? justccell_form_recipients('newsletter')
    ...
    justccell_send_lead_mail($recipients, $subject . ' ' . $email, 'New footer signup: ' . $email, $email);
