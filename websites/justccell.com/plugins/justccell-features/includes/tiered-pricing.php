 ...
 */
function justccell_tiered_pricing_rows_to_display(array $rows): array
{
    $out = [];
    foreach ($rows as $row) {
        if (!is_array($row)) {
 ...
 * @return list<array{range:string,price:string,qty_min:int,qty_max:int,unit:float}>
 */
function justccell_tiered_pricing_single_band_display_row(float $unit): array
{
    if ($unit <= 0) {
        return [];
    }
...
/**
 * Woo catalog unit for a variation (regular/sale), excluding parent tier fallbacks.
 */
function justccell_tiered_pricing_variation_catalog_unit(WC_Product_Variation $variation): float
{
    $unit = (float) $variation->get_price('edit');
    if ($unit <= 0) {
        $unit = (float) $variation->get_regular_price('edit');
    }
    if ($unit <= 0) {
        $unit = (float) $variation->get_sale_price('edit');
    }
...
/**
 * Shift parent volume bands when a variation has its own catalog base price.
 *
 * @param list<array{min_qty:int,max_qty:int,price:float}> $parent_rows
 * @return list<array{min_qty:int,max_qty:int,price:float}>
 */
function justccell_tiered_pricing_rows_with_variation_offset(array $parent_rows, float $variation_unit): array
{
    if ($parent_rows === [] || $variation_unit <= 0) {
        return $parent_rows;
    }

    $first = (float) ($parent_rows[0]['price'] ?? 0);
    if ($first <= 0) {
        return $parent_rows;
    }

    $delta = round($variation_unit - $first, 4);
    if (abs($delta) < 0.0001) {
        return $parent_rows;
    }
    ...
    foreach ($parent_rows as $row) {
        ...
        $out[] = [
            'min_qty' => (int) ($row['min_qty'] ?? 1),
            'max_qty' => (int) ($row['max_qty'] ?? 0),
            'price'   => max(0.01, round((float) ($row['price'] ?? 0) + $delta, 4)),
        ];
    }
...
/**
 * Resolved tier rows for a variation (meta → offset parent bands → parent bands → single Woo price).
 *
 * @return list<array{min_qty:int,max_qty:int,price:float}>
 */
function justccell_tiered_pricing_resolve_rows_for_variation(int $variation_id): array
{
    ...
    $own = justccell_tiered_pricing_resolve_rows($variation_id, false);
    if ($own !== []) {
        return $own;
    }

    $parent_id = (int) $variation->get_parent_id();
    $parent_rows = $parent_id > 0
        ? justccell_tiered_pricing_resolve_rows($parent_id, true)
        : [];

    $catalog_unit = justccell_tiered_pricing_variation_catalog_unit($variation);
    if ($parent_rows !== [] && $catalog_unit > 0) {
        return justccell_tiered_pricing_rows_with_variation_offset($parent_rows, $catalog_unit);
    }
    if ($parent_rows !== []) {
        return $parent_rows;
    }
...
function justccell_tiered_pricing_display_rows(int $product_id): array
{
    ...
    return justccell_tiered_pricing_rows_to_display(
        justccell_tiered_pricing_resolve_rows($product_id, true)
    );
}

/**
 * Per-variation buy-box rows: variation tier meta first, else Woo catalog price, never kit defaults.
 *
 * @return list<array{range:string,price:string,qty_min:int,qty_max:int,unit:float}>
 */
...
function justccell_tier_unit_price_for_qty(int $product_id, int $variation_id, int $qty): ?float
{
    ...
    if ($variation_id > 0) {
        ...
        $tiers = justccell_tiered_pricing_resolve_rows($product_id, true);
    }
...
add_action('woocommerce_process_product_meta', static function (int $product_id): void {
    ...
    if (function_exists('justccell_wc_clear_variable_product_transients') && function_exists('wc_get_product')) {
        $product = wc_get_product($product_id);
        if ($product instanceof WC_Product_Variable) {
            justccell_wc_clear_variable_product_transients($product_id);
        }
    }
