...
function justccell_product_pages(): array
{
    ...
    if (!function_exists('justccell_product_pages_data')) {
        require_once JUSTCCELL_FEATURES_DIR . 'includes/product-data.php';
    }
...
/**
 * Infer storefront category (All-In-Ones, Pod Systems, …) from slug when Woo
 * product_cat only has CCELL 3.0 and no storefront term.
 */
function justccell_storefront_category_from_slug(string $slug): string
{
    if ($slug === '') {
        return '';
    }
    ...
    if (function_exists('justccell_j3_product_groups_defaults')) {
        foreach (justccell_j3_product_groups_defaults() as $group) {
            ...
            if ($cat === '' || !array_key_exists($cat, $labels)) {
                continue;
            }
            foreach ((array) ($group['slugs'] ?? []) as $base) {
                $base = (string) $base;
                if ($base !== '' && $base === $slug) {
                    return $cat;
                }
    ...
    if (function_exists('justccell_product_pages_data')) {
        $seed = justccell_product_pages_data()[$slug] ?? null;
        if (is_array($seed)) {
            $cat = (string) ($seed['category'] ?? '');
            if ($cat !== '' && array_key_exists($cat, $labels)) {
                return $cat;
            }
        }
...
/**
 * Resolve the storefront product_cat slug used in /{category}/{slug}/ URLs.
 */
function justccell_product_storefront_category(int $product_id, string $slug = ''): string
{
    $labels = justccell_product_category_labels();
    ...
    return justccell_storefront_category_from_slug($slug);
}
...
function justccell_product_url(string $slug): string
{
    ...
    if (function_exists('justccell_woo_product_id_by_slug')) {
        ...
        if ($pid > 0) {
            $cat = justccell_product_storefront_category($pid, $slug);
            if ($cat !== '') {
                return home_url('/' . $cat . '/' . trim($slug, '/') . '/');
            }
        }
    }

    $inferred = justccell_storefront_category_from_slug($slug);
    if ($inferred !== '') {
        return home_url('/' . $inferred . '/' . trim($slug, '/') . '/');
    }
