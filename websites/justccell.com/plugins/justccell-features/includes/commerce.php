...
function justccell_product_buy_box(string $slug, int $product_id = 0): array
{
    ...
    $variation_tiers = [];
    if ($product_id > 0 && function_exists('wc_get_product') && function_exists('justccell_tiered_pricing_display_rows_for_variation')) {
        $wc_product = wc_get_product($product_id);
        if ($wc_product instanceof WC_Product && $wc_product->is_type('variable')) {
            foreach ($wc_product->get_children() as $child_id) {
                $child_id   = (int) $child_id;
                $child_rows = justccell_tiered_pricing_display_rows_for_variation($child_id);
