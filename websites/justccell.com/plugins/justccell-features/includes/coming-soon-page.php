 ...
 * Use location rule_match only — returning false from acf/load_field_group breaks ACFML.
 *
 * @param mixed $match
 ...
 * @return mixed
 */
add_filter('acf/location/rule_match', static function ($match, $rule, $screen, $field_group) {
    unset($rule);
    if (!is_array($field_group) || !is_array($screen)) {
        return $match;
    }
    ...
    if (isset($screen['post_id'])) {
        ...
        if (is_numeric($raw)) {
            $post_id = (int) $raw;
        } elseif (is_string($raw) && str_starts_with($raw, 'post_')) {
            $post_id = (int) substr($raw, 5);
        }
    }
