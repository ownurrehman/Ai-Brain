...
function justccell_listing_seed_pages_acf_content(): void
{
    ...
    foreach (justccell_listing_defaults() as $slug => $row) {
        ...
        $existing_sections = get_field('listing_product_sections', $page_id);
        if (!is_array($existing_sections) || $existing_sections === []) {
            $section_defaults = function_exists('justccell_listing_product_section_defaults')
                ? justccell_listing_product_section_defaults($slug)
                : [];
            if ($section_defaults !== []) {
                $section_rows = [];
                foreach ($section_defaults as $section) {
                    $section_rows[] = [
                ...
                update_field('listing_product_sections', $section_rows, $page_id);
            }
        }
    }
...
/**
 * One-time seed: All-In-Ones section headings (products picked manually in wp-admin).
 */
function justccell_listing_seed_product_sections_once(): void
{
    if (get_option('justccell_listing_sections_seed_v1') === '1') {
        return;
    }
    if (!function_exists('get_field') || !function_exists('update_field')) {
        return;
    ...
    foreach (array_keys(justccell_listing_defaults()) as $slug) {
        $page_id = justccell_listing_page_id($slug);
        if ($page_id < 1) {
            continue;
        }
        $existing = get_field('listing_product_sections', $page_id);
        if (is_array($existing) && $existing !== []) {
            continue;
        }
        $defaults = function_exists('justccell_listing_product_section_defaults')
            ? justccell_listing_product_section_defaults($slug)
            : [];
        if ($defaults === []) {
            continue;
        ...
        update_field('listing_product_sections', $rows, $page_id);
    }

    update_option('justccell_listing_sections_seed_v1', '1', false);
}

add_action('init', 'justccell_home_seed_page_acf_content', 22);
add_action('init', 'justccell_listing_seed_pages_acf_content', 22);
add_action('init', 'justccell_listing_seed_product_sections_once', 23);
