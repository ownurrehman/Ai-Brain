...
/**
 * Resolve a nav menu item post ID from an ACF field context (Appearance → Menus uses string IDs).
 */
function justccell_acf_nav_menu_item_id_from_field(array $field): int
{
    $post_id = $field['post_id'] ?? 0;
    ...
    if (function_exists('acf_decode_post_id')) {
        ...
        if (is_array($decoded)) {
            ...
            $id   = (int) ($decoded['id'] ?? 0);
            if ($id > 0 && ($type === 'nav_menu_item' || $type === 'menu_item')) {
                return $id;
            }
            if ($id > 0 && $type === 'post') {
                $raw = get_post($id);
    ...
    if (is_string($post_id) && preg_match('/(?:nav_menu_item|menu_item)_(\d+)/', $post_id, $matches)) {
...
/**
 * True when a nav menu item belongs to the Primary (header) menu location.
 */
function justccell_nav_menu_item_in_primary_menu(int $item_id): bool
{
    if ($item_id < 1 || !taxonomy_exists('nav_menu')) {
        return false;
    }
    ...
    return in_array($primary_id, array_map('intval', $menu_ids), true);
}
...
function justccell_product_in_storefront_category(int $product_id, string $category): bool
{
    ...
    // CCELL 3.0-only SKUs: infer storefront tab from slug / J3 defaults when no Woo term exists.
    if (!function_exists('justccell_product_has_j3_category') || !justccell_product_has_j3_category($product_id)) {
        return false;
    }
    if (!function_exists('justccell_product_storefront_category')) {
        return false;
    }
    ...
    if (is_array($terms)) {
        foreach ($terms as $cslug) {
            if (array_key_exists($cslug, $labels)) {
                return false;
            }
        }
    }
    ...
    return justccell_product_storefront_category($product_id, $slug) === $category;
}
