    ...
    if (taxonomy_exists('product_cat')) {
        ...
        if (is_array($terms)) {
            foreach ($terms as $term) {
                ...
                $name = strtolower(trim(html_entity_decode($term->name, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
                if ($name === '') {
                    continue;
                }
                if (
                    preg_match('/\b(?:ccell|just\s*ccell|cell)\s*3(?:\.0)?\b/u', $name) === 1
                    || preg_match('/\b3(?:\.0)?\s*(?:ccell|just\s*ccell)\b/u', $name) === 1
                ) {
...
/**
 * True when the product has a CCELL 3.0 WooCommerce category assigned (menu + rail source of truth).
 */
function justccell_product_has_j3_category(int $product_id): bool
{
...
/**
 * True when a product is flagged as CCELL 3.0 (Woo category, legacy meta, or retired ACF toggle).
 */
function justccell_product_is_j3(int $product_id): bool
{
    ...
    if (justccell_product_has_j3_category($product_id)) {
        return true;
    }
...
/**
 * Published CCELL 3.0 SKUs in a storefront category (menu_order).
 * Requires both the storefront category and a CCELL 3.0 Woo product category.
 *
 * @return list<array<string, mixed>>
 */
function justccell_j3_items_from_category(string $category): array
{
    if ($category === '' || !function_exists('wc_get_products')) {
        return [];
    }
    ...
    $j3_slugs = justccell_j3_product_cat_slugs();
    if ($j3_slugs === []) {
        return [];
    }
    ...
    foreach ($ids as $product_id) {
        ...
        if (function_exists('justccell_product_in_storefront_category')
            && !justccell_product_in_storefront_category($product_id, $category)
        ) {
            continue;
        }
        ...
        $slug = (string) ($product->get_slug() !== '' ? $product->get_slug() : $product->get_sku());
        if ($slug === '') {
            continue;
        }
...
/**
 * Header mega cards from explicit product picks — J3 category + storefront tab required.
 *
 * @param list<int> $product_ids
 * @return list<array{name:string,url:string,image:string,image_id:int}>
 */
function justccell_j3_mega_cards_from_product_ids(string $category, array $product_ids, int $limit = 5): array
{
