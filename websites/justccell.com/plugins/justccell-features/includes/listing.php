...
/**
 * Storefront category slug for a catalog listing page (empty on hub pages).
 */
function justccell_listing_page_category_slug(int $page_id): string
{
    if ($page_id < 1) {
        return '';
    }
    $slug = (string) get_post_field('post_name', $page_id);
    return array_key_exists($slug, justccell_product_category_labels()) ? $slug : '';
}

/**
 * Default section copy for All-In-Ones (ccell-style groups). Headings only — products are picked in wp-admin.
 *
 * @return list<array{heading:string,description:string}>
 */
function justccell_listing_product_section_defaults(string $category): array
{
    if ($category !== 'all-in-ones') {
        return [];
    }
    ...
    return [
        [
            'heading'     => __('Best For Distillates', 'justccell'),
            'description' => __('Made with classic SE heating elements that unleash flavor and vapor from cannabis oils, especially distillates.', 'justccell'),
        ],
        [
            'heading'     => __('Best For Live Rosins', 'justccell'),
            'description' => __('Crafted with rosin-dedicated heating elements designed for a flavorful, uninterrupted rosin consumption experience.', 'justccell'),
        ],
        [
            'heading'     => __('Best For Live Resins', 'justccell'),
            'description' => __('Built with EVO heating elements that unlock flavorful hits and big clouds. Most compatible with high-terpene extracts and live resins.', 'justccell'),
        ],
        [
            'heading'     => __('All-Oil-Capable', 'justccell'),
            'description' => __('Engineered with advanced heating elements for a true-to-strain experience with all types of cannabis oils, including live rosins and liquid diamonds.', 'justccell'),
        ],
    ];
}

/**
 * Resolve catalog card items from Woo product IDs, preserving picker order.
 *
 * @param list<int> $product_ids
 * @return list<array<string, mixed>>
 */
function justccell_catalog_items_from_product_ids(array $product_ids, string $category): array
{
    if ($product_ids === [] || $category === '') {
        return [];
    }
    ...
    foreach ($product_ids as $product_id) {
        ...
        if (isset($by_woo[$product_id])) {
            $out[] = $by_woo[$product_id];
        }
    }
...
/**
 * ACF-driven catalog section groups for one category listing page.
 *
 * @return list<array{title:string,copy:string,heading_tag:string,items:list<array<string, mixed>>}>
 */
function justccell_listing_product_section_groups(int $page_id, string $category): array
{
    if ($page_id < 1 || $category === '' || !function_exists('get_field')) {
        return [];
    }

    $rows = get_field('listing_product_sections', $page_id);
    if (!is_array($rows) || $rows === []) {
        return [];
    }
    ...
    foreach ($rows as $row) {
        ...
        $product_ids = [];
        foreach ((array) ($row['products'] ?? []) as $picked) {
            if (is_numeric($picked)) {
                $product_ids[] = (int) $picked;
            } elseif ($picked instanceof WP_Post) {
                $product_ids[] = (int) $picked->ID;
            }
        }

        $items = justccell_catalog_items_from_product_ids($product_ids, $category);
        if ($items === []) {
            continue;
        }

        $heading_tag = sanitize_key((string) ($row['heading_tag'] ?? 'h2'));
        ...
        $groups[] = [
            'title'       => $heading,
            'copy'        => trim((string) ($row['description'] ?? '')),
...
/**
 * Default 3-image spotlight keys per category (ccell.com sub_img).
 *
 * @return array<string, array{wide:string,left:string,right:string}>
 */
function justccell_listing_spotlight_defaults(): array
{
    return [
        'all-in-ones' => [
            'wide'  => 'public_uploads_images_20220722_8a92dffa0ebb4edacb6b0435f71729ec.jpg',
            'left'  => 'public_uploads_images_20250226_99e934d87a73ce293e52dc447ee19989.jpg',
            'right' => 'public_uploads_images_20211104_164423d1cde166b90adf19e498022a9e.jpg',
        ],
    ];
}

/**
 * @return array{wide_id:int,left_id:int,right_id:int,wide_key:string,left_key:string,right_key:string}
 */
function justccell_listing_spotlight_from_acf(int $page_id): array
{
    $empty = [
        'wide_id'   => 0,
        'left_id'   => 0,
    ...
    $to_id = static function ($value): int {
        if (function_exists('justccell_acf_to_attachment_id')) {
            return justccell_acf_to_attachment_id($value);
        }
        if (is_array($value)) {
    ...
    return [
        'wide_id'   => $to_id(get_field('listing_spotlight_wide', $page_id)),
        'left_id'   => $to_id(get_field('listing_spotlight_left', $page_id)),
        'right_id'  => $to_id(get_field('listing_spotlight_right', $page_id)),
        'wide_key'  => '',
        'left_key'  => '',
        'right_key' => '',
    ];
...
/**
 * Lifestyle 3-image block for catalog pages (wide + two-up), or null when unset.
 *
 * @return array{wide_id:int,left_id:int,right_id:int,wide_key:string,left_key:string,right_key:string}|null
 */
function justccell_listing_spotlight(int $page_id): ?array
{
    if ($page_id < 1) {
        return null;
    }

    $spot = justccell_listing_spotlight_from_acf($page_id);
    if ($spot['wide_id'] > 0 && ($spot['left_id'] > 0 || $spot['right_id'] > 0)) {
        return $spot;
    }

    $category = justccell_listing_page_category_slug($page_id);
    if ($category !== '' && function_exists('justccell_seed_listing_spotlight_fields')) {
        justccell_seed_listing_spotlight_fields($category);
        $spot = justccell_listing_spotlight_from_acf($page_id);
        if ($spot['wide_id'] > 0 && ($spot['left_id'] > 0 || $spot['right_id'] > 0)) {
            return $spot;
        }
    ...
    $defaults = justccell_listing_spotlight_defaults()[$category] ?? null;
    if (!is_array($defaults)) {
        return null;
    }

    justccell_ensure_media_files(array_values($defaults));
    $wide_id  = justccell_media_id((string) ($defaults['wide'] ?? ''));
    $left_id  = justccell_media_id((string) ($defaults['left'] ?? ''));
    $right_id = justccell_media_id((string) ($defaults['right'] ?? ''));

    if ($wide_id < 1 || ($left_id < 1 && $right_id < 1)) {
        return [
            'wide_id'   => $wide_id,
            'left_id'   => $left_id,
            'right_id'  => $right_id,
            'wide_key'  => (string) ($defaults['wide'] ?? ''),
            'left_key'  => (string) ($defaults['left'] ?? ''),
            'right_key' => (string) ($defaults['right'] ?? ''),
        ];
    }
...
function justccell_seed_listing_spotlight_fields(string $category): void
{
    ...
    $defaults = justccell_listing_spotlight_defaults()[$category] ?? null;
    $page_id  = justccell_listing_page_id($category);
    if ($defaults === null || $page_id < 1) {
        return;
    }
    if (justccell_listing_spotlight_from_acf($page_id)['wide_id'] > 0) {
        return;
    }

    justccell_ensure_media_files(array_values($defaults));
    $wide  = justccell_media_id((string) $defaults['wide']);
    $left  = justccell_media_id((string) $defaults['left']);
    $right = justccell_media_id((string) $defaults['right']);
    if ($wide < 1) {
        return;
    }

    update_field('listing_spotlight_wide', $wide, $page_id);
    if ($left > 0) {
        update_field('listing_spotlight_left', $left, $page_id);
    }
    if ($right > 0) {
        update_field('listing_spotlight_right', $right, $page_id);
    }
}
 ...
 */
function justccell_listing_section_products_relationship_query(array $args, $field, $post_id): array
{
    unset($field);
    $page_id = (int) $post_id;
    if ($page_id < 1 || !taxonomy_exists('product_cat')) {
        return $args;
    }
...
add_filter(
    'acf/fields/relationship/query/key=field_jc_listing_section_products',
    'justccell_listing_section_products_relationship_query',
    10,
    3
);
...
add_action('wp', static function (): void {
    ...
    if ($cat !== '' && isset(justccell_listing_defaults()[$cat])) {
        ...
        justccell_seed_listing_spotlight_fields($cat);
        return;
    }
    if (is_front_page()) {
