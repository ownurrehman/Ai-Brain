...
/**
 * Catalog listing groups — ACF sections or flat fallback.
 *
...
{
    $page_id = function_exists('justccell_listing_page_id') ? justccell_listing_page_id($category) : 0;
    if ($page_id > 0 && function_exists('justccell_listing_product_section_groups')) {
        $sections = justccell_listing_product_section_groups($page_id, $category);
        if ($sections !== []) {
            return $sections;
        }
    }
