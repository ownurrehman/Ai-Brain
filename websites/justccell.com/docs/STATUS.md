...
- Custom theme clone of the manufacturer storefront: home, catalog, product, About, Contact, Products mega, footer. **Just CCELL 3.0** canonical URL is **`/justccell-3-0/`** (legacy `/ccell-3-0/` 301s here). Checklist: [design-clone.md](design-clone.md).
...
- Product buy box: Quantity / Per Item Price table, Colour + Select a Combination dropdowns, quantity stepper, purple Add to basket (quote until payments on). Total is the hero; active tier row follows qty. Laser engraving editor lives in the buy box when enabled.
...
- Product PDP heading ladder (theme 0.9.197+): **Product heading** = sole `<h1>`; **Product Tagline** = `<h2>`; **Specs** = `<h3>` + semantic `<ul>`. Banner heading / Banner text ACF fields removed. Woo Product description is the long-copy editor (H2/H3/lists).
...
- Packaging + Elite Terpenes use **Justccell Coming Soon**. Laser-engraving page is published. Bio heating page is **`/justccell-3-0/`**.
