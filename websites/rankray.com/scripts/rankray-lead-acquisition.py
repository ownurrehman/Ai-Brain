...
def search_google_places(query, location, api_key):
    ...
    params = {
        'query': "{} in {}".format(query, location),
        'key': api_key,
        'type': 'business'
    }
    ...
    try:
        ...
        if data.get('status') == 'OK':
            ...
            print("Places API error: {}".format(data.get('status')))
            return []
    except Exception as e:
        print("Error searching places: {}".format(e))
        return []
...
def generate_lead_id(existing_ids):
    ...
    while True:
        ...
        lead_id = "RR-{}-{}".format(date_str, suffix)
...
def assess_lead_quality(place):
    ...
    if rating < 4.0:
        score += 30
        reasons.append("Low rating: {}".format(rating))
...
def find_leads_batch(sheets_service, places_api_key, batch_size=20):
    ...
    while len(leads) < batch_size and attempts < max_attempts:
        ...
        print("Searching: {} in {}...".format(industry, city))
        ...
        for place in places:
            ...
            lead = [
                ...
                '',  # Follow-up Status
                '',  # Follow-up Date
                "Found via Google Places. Score: {}/100".format(score)  # Notes
            ]
        ...
        print("  Found {} leads so far".format(len(leads)))
...
def append_leads_to_sheet(sheets_service, leads):
    ...
    print("Appended {} leads starting at row {}".format(len(leads), next_row))
    return result
