...
- User: Dan Torres (ID: 10, slug: dan, email: Dan)
- Role: administrator
...
3. **App password username mismatch:** The env var `TONIC_PHYSIO_REST_USER` is set to "Openclaw" but the working app password is tied to the email-based user "Dan". The "Openclaw" username auth returns 401. Either the app password was revoked for that user, or the username is incorrect.
...
2. **Fix app password for "Openclaw" user:** The `TONIC_PHYSIO_REST_USER=Openclaw` credential is not working. Either regenerate the app password for that user in wp-admin, or update the .env to use `Dan` as the REST user (which works).
